...
function setBorrowFormState(){
  ...
  const entry = PlifeOSStorage.getBorrow().find((item) => item.id === borrowState.editingId)
...
function saveBorrowEntry(){
  ...
  if(!person){
    PlifeOSFeedback.error(tr("messages.enterPersonFirst", "Enter a person name first."))
    return
  }
  ...
  if(borrowState.editingId){
    PlifeOSStorage.updateBorrow(borrowState.editingId, {person, amount})
    PlifeOSFeedback.success(tr("messages.borrowUpdated", "Borrowed entry updated."))
  }else{
    PlifeOSStorage.addBorrow({person, amount})
    PlifeOSFeedback.success(tr("messages.borrowAdded", "Borrowed entry added."))
  }
...
function loadBorrow(resetPage){
  ...
  const data = PlifeOSStorage.getBorrow().slice().sort((left, right) => {
...
function deleteBorrow(id){
  const entry = PlifeOSStorage.getBorrow().find((item) => item.id === id)

  if(!entry){
    return
  }

  PlifeOSStorage.removeBorrow(id)
  ...
  loadBorrow()
  refreshDashboard()
  PlifeOSFeedback.show(tr("borrow.deleted", "Deleted {value}.", {value: entry.person}), {
    type: "info",
    actionLabel: tr("common.undo", "Undo"),
    onAction: () => {
      PlifeOSStorage.addBorrow(entry)
      loadBorrow()
      refreshDashboard()
      PlifeOSFeedback.success(tr("borrow.restored", "Borrowed entry restored."))
    }
  })
}
